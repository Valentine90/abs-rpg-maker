...
#------------------------------------------------------------------------------
# Este script contém o sistema de batalha.
#------------------------------------------------------------------------------
# Autor: Valentine
# Versão: 2.0
...
class Game_Battler
  ...
  def attack_effect(attacker)
    ...
    # Se houver falha
      ...
      self.damage = Configs::DAMAGE_TEXTS[:miss]
      self.critical = false
    end
    return true
  end
  ...
  def skill_effect(user, skill)
    ...
    if hit_result == true
      ...
      if skill.power == 0
        ...
        self.damage = Configs::DAMAGE_TEXTS[:miss] unless @state_changed
      end
    # Se errar
    else
      self.damage = Configs::DAMAGE_TEXTS[:miss]
...
class Game_Actor < Game_Battler
  ...
  def exp=(exp)
    ...
    if @level > last_level and $scene.is_a?(Scene_Map)
      ...
      self.damage = Configs::DAMAGE_TEXTS[:level_up]
      recover_all
    end
  end
end
...
class Game_Character
  ...
  def skill_attack_range(skill)
    range = Configs::RANGE_SKILLS[skill.id]
  ...
  def skill_explode_range(skill)
    explosive = Configs::RANGE_EXPLOSIVES[skill.id]
  ...
  def skill_recover(skill)
    animate_attack(Configs::SKILL_ANIMATIONS[skill.id]) if Configs::SKILL_ANIMATIONS.has_key?(skill.id)
  ...
  def update_states
    for i in @actor.states
      next if !Configs::STATES_TIME[i] or Configs::STATES_TIME[i] == 0
      if @state_time <= Configs::STATES_TIME[i] * 40
        @state_time += 1
        # Causar dano
...
class Game_Event < Game_Character
  ...
  def kill
    #return if @respawn
    @respawn = Configs::ENEMY_RESPAWN_TIME * 40
  ...
  def treasure
    ...
    if (gold = rand(@actor.gold)) > 0
      ...
      $game_player.reward = gold if Configs::GOLD_ICON.empty?
    end
  ...
  #--------------------------------------------------------------------------
  def enemy_drop
    if $game_drop.size >= Configs::MAX_MAP_DROPS
  ...
  def conditions_met?(action)
    ...
    return false if Graphics.frame_count % (@aggressiveness * Configs::ATTACK_TIME) != 0
    return true
  end
  #--------------------------------------------------------------------------
  ...
  def attack_skill(skill)
    ...
    when 1 # Herói
      ...
      if Configs::RANGE_SKILLS.has_key?(skill.id)
        return unless in_direction?(@target)
        skill_attack_range(skill)
      # Se é um explosivo de longo alcance
      elsif Configs::RANGE_EXPLOSIVES.has_key?(skill.id)
        return unless in_direction?(@target)
        skill_explode_range(skill)
      # Se a habilidade ataca em área
      elsif Configs::AREA_SKILLS.has_key?(skill.id)
        return unless in_range?(@target, Configs::AREA_SKILLS[skill.id])
  ...
  def skill_attack_area(skill)
    animate_attack(Configs::SKILL_ANIMATIONS[skill.id]) if Configs::SKILL_ANIMATIONS.has_key?(skill.id)
    hit_player(skill.animation2_id, skill.id) if in_range?($game_player, Configs::AREA_SKILLS[skill.id])
    for event in $game_map.events.values
      next if !event.actor? or event.actor.dead? or event == self
      next unless in_range?(event, Configs::AREA_SKILLS[skill.id])
...
class Game_Player < Game_Character
  ...
  #--------------------------------------------------------------------------
  def update_attack
    return unless Input.press?(Configs::ATTACK_KEY)
    return unless @actor.weapon_id > 0
    # Se for impossível atacar
    return unless @actor.restriction != 4
    @weapon_attack_time = Configs::COOLDOWN_WEAPONS.has_key?(@actor.weapon_id) ? Configs::COOLDOWN_WEAPONS[@actor.weapon_id] : Configs::ATTACK_TIME
    if Configs::RANGE_WEAPONS.has_key?(@actor.weapon_id)
      attack_range
    else
      attack_normal
    end
  ...
  #--------------------------------------------------------------------------
  def update_defense
    if Input.press?(Configs::DEFEND_KEY) and @actor.armor1_id > 0
  ...
  def update_hotkeys
    ...
    for key in @actor.hotkeys.keys
      ...
      @skill_attack_time = Configs::COOLDOWN_SKILLS
  ...
  def attack_normal
    for event in $game_map.events.values
      ...
      animate_attack(Configs::MELEE_ANIMATIONS[@actor.weapon_id]) if Configs::MELEE_ANIMATIONS.has_key?(@actor.weapon_id)
  ...
  #--------------------------------------------------------------------------
  def attack_range
    return if $game_party.item_number(Configs::RANGE_WEAPONS[@actor.weapon_id][:item_id]) == 0
    range = Configs::RANGE_WEAPONS[@actor.weapon_id]
  ...
  def skill_attack_area(skill)
    animate_attack(Configs::SKILL_ANIMATIONS[skill.id]) if Configs::SKILL_ANIMATIONS.has_key?(skill.id)
    for event in $game_map.events.values
      next if !event.actor? or event.actor.dead? or !in_range?(event, Configs::AREA_SKILLS[skill.id])
      hit_enemy(event, skill.animation2_id, skill.id)
    end
  end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def defend
    animate_attack(Configs::DEFEND_ANIMATIONS[@actor.armor1_id]) if Configs::DEFEND_ANIMATIONS.has_key?(@actor.armor1_id)
    @actor.current_action.basic = 1
  end
  #--------------------------------------------------------------------------
  # * Usar item
  ...
  def auto_recovery
    @recover_time += 1
    if @recover_time == Configs::RECOVER_TIME * 40
      @actor.hp += (@actor.agi / 100).next * Configs::RECOVER_HP if @actor.hp != @actor.maxhp
      @actor.sp += (@actor.agi / 100).next * Configs::RECOVER_SP if @actor.sp != @actor.maxsp
      @recover_time = 0
    end
  end
  #--------------------------------------------------------------------------
...
class Game_Range
  ...
  #     step_anime      : animação
  #     skill_id        : ID da habilidade
  #     range_explosive : alcance do explosivo
  #--------------------------------------------------------------------------
  def initialize(parent, picture_name, animation, move_speed, range, step_anime, skill_id = 0, range_explosive = 0)
    ...
    @skill_id = skill_id
    @range_explosive = range_explosive
  ...
  #--------------------------------------------------------------------------
  def explode(user)
    user.hit_player($data_skills[@skill_id].animation2_id, @skill_id) if @event and user.in_range?($game_player, @range_explosive)
    for event in $game_map.events.values
      next if !event.actor? or event.actor.dead? or event == user or !user.in_range?(event, @range_explosive)
...
class Game_Drop
  ...
  def update
    @time += 1
    @destroy = (@time >= Configs::DROP_DESPAWN_TIME * 40)
  end
end
...
class Sprite_Character < RPG::Sprite
  ...
  def damage(value, critical)
    ...
    @_damage_sprite.bitmap.font.name = Configs::DAMAGE_FONT_NAME
    @_damage_sprite.bitmap.font.bold = Configs::DAMAGE_FONT_BOLD
    @_damage_sprite.bitmap.font.size = Configs::DAMAGE_FONT_SIZE
    if critical
      draw_damage_text(Configs::DAMAGE_TEXTS[:critical], Configs::DAMAGE_COLORS[:critical])
    else
      ...
      draw_damage_text(value.to_s, Configs::DAMAGE_COLORS[color_key])
    end
  ...
  def damage_color_key(value)
    ...
    # Cura
      ...
      :heal
    # Errou
    elsif value == Configs::DAMAGE_TEXTS[:miss]
      :miss
    # Subiu nível
    elsif value == Configs::DAMAGE_TEXTS[:level_up]
      :level_up
    # Experiência
    else
      :exp
  ...
  def show_reward(value)
    dispose_reward
    bitmap = RPG::Cache.icon(Configs::GOLD_ICON)
...
class Spriteset_Map
  ...
  def update_characters
    ...
    for drop in $game_drop
      next unless drop
      if drop.draw
        if Input.trigger?(Configs::GET_DROP_KEY) && drop.x == $game_player.x and drop.y == $game_player.y
          drop.get_reward
          break
        end
        drop.update
